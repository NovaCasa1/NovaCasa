...
// ─────────────────────────────────────────────
//  Datos de ejemplo (se mezclan con los de la BD)
// ─────────────────────────────────────────────
const _featuredProperties = [
  Property(
    ...
    beds: 4, baths: 3, area: '220 m²', isFeatured: true,
  ),
  Property(
    ...
    beds: 5, baths: 4, area: '380 m²', isFeatured: true,
  ),
  Property(
    ...
    beds: 3, baths: 2, area: '110 m²', isFeatured: true,
  ),
];

const _recentProperties = [
  Property(
    title: 'Apartamento Moderno', location: 'Sevilla, Triana', price: '€ 185.000',
    imageUrl: 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=600',
    beds: 2, baths: 1, area: '75 m²',
  ),
  Property(
    title: 'Casa Adosada', location: 'Málaga, El Limonar', price: '€ 310.000',
    imageUrl: 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600',
    beds: 3, baths: 2, area: '145 m²',
  ),
  Property(
    title: 'Estudio Renovado', location: 'Bilbao, Casco Viejo', price: '€ 98.000',
    imageUrl: 'https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?w=600',
    beds: 1, baths: 1, area: '42 m²',
  ),
  Property(
    title: 'Chalet Independiente', location: 'Zaragoza, Las Lomas', price: '€ 560.000',
    imageUrl: 'https://images.unsplash.com/photo-1570129477492-45c003edd2be?w=600',
    beds: 5, baths: 3, area: '300 m²',
  ),
];

// ─────────────────────────────────────────────
//  HomePage principal — recibe el usuario logueado
// ─────────────────────────────────────────────
class HomePage extends StatefulWidget {
  final UserModel user; // ← usuario del login/register
...
class _HomePageState extends State<HomePage> {
  ...
  List<dynamic> _dbDwellings = []; // propiedades de la BD
  ...
  // Carga propiedades reales de la BD
  Future<void> _loadDwellings() async {
    try {
      final response = await http.get(Uri.parse('http://localhost:3000/api/dwellings'));
      if (response.statusCode == 200) {
        setState(() => _dbDwellings = jsonDecode(response.body));
      }
    } catch (e) {
      // Si no hay conexión usa los datos de ejemplo
    }
  }
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Icon(items[i]['icon'] as IconData,
                    color: selected ? _blue : _grey, size: 24),
              ),
            );
          }),
          const Spacer(),
  ...
  // ── TOP BAR — muestra el nombre real del usuario ───────────────────────────
  Widget _buildTopBar() {
  ...
  Widget _buildSectionHeader(String title, {required VoidCallback onSeeAll}) {
    return Row(
      children: [
        Text(title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: _dark)),
        const Spacer(),
        GestureDetector(
          onTap: onSeeAll,
  ...
  Widget _buildRecentGrid() {
    // Si hay propiedades en la BD las usa, si no usa las de ejemplo
    final properties = _dbDwellings.isNotEmpty ? null : _recentProperties;

    if (_dbDwellings.isNotEmpty) {
      return GridView.builder(
        shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
        gridDelegate: const SliverGridDelegateWithMaxCrossAxisExtent(
          maxCrossAxisExtent: 320, childAspectRatio: 0.85,
          crossAxisSpacing: 20, mainAxisSpacing: 20,
        ),
        itemCount: _dbDwellings.length,
        itemBuilder: (_, i) => _DbDwellingCard(dwelling: _dbDwellings[i]),
      );
    }
  ...
  // ── CERRAR SESIÓN → vuelve a HomePublicPage ────────────────────────────────
  void _confirmLogout() {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Text('¿Seguro que quieres salir, ${widget.user.name}?'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(builder: (_) => const HomePublicPage()),
...
// ─────────────────────────────────────────────
//  Tarjeta — Propiedad de la BD
// ─────────────────────────────────────────────
class _DbDwellingCard extends StatelessWidget {
  final Map<String, dynamic> dwelling;
  const _DbDwellingCard({required this.dwelling});
          ...
          children: [
            ...
            if (dwelling['city'] != null)
              Row(children: [
                ...
                Expanded(
                  child: Text('${dwelling['city']['name']}, ${dwelling['city']['country']}',
            ...
            Row(children: [
              ...
              Text('${dwelling['rooms']}', style: const TextStyle(color: Color(0xFF8A94A6), fontSize: 12)),
              const SizedBox(width: 12),
              const Icon(Icons.straighten, color: Color(0xFF8A94A6), size: 14),
              const SizedBox(width: 4),
              Text('${dwelling['meters']} m²', style: const TextStyle(color: Color(0xFF8A94A6), fontSize: 12)),
            ]),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Icon(_saved ? Icons.favorite : Icons.favorite_border,
                      color: _saved ? Colors.redAccent : Colors.white, size: 18),
                ),
              ),
            ]),
            const Spacer(),
                ...
                Positioned(
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Icon(_saved ? Icons.favorite : Icons.favorite_border,
                          color: _saved ? Colors.redAccent : Colors.grey, size: 16),
                    ),
                  ),
                ),
              ],
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                Text(p.price,
                    style: const TextStyle(color: Color(0xFF007BFF), fontSize: 16, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ],
